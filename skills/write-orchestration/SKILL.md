...
5. **Check the architecture.** Read `specs/architecture.yaml` for existing **implementation modules** to inject as clients into the orchestration. The orchestration itself must live in a **dedicated orchestration module** — if none exists for this feature area, name one; it will be recorded in the architecture once the code is written. See [docs/specs/code.md](docs/specs/code.md).
